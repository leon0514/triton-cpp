...
#define BACKEND_NAME "yolo11_seg_postprocess"
...
#define RETURN_TRITON_ERROR(CODE, MSG) \
    ...
    struct ModelState
    {
        ...
        TRITONSERVER_Error *LoadConfig()
        {
            ...
            TRITONSERVER_Error *err = ParseYolo11SegPostprocessConfig(config_message, config);
    ...
    struct ModelInstanceState
    {
        ...
        int device_id = 0;
        std::unique_ptr<yolo11_seg_postprocess::Yolo11SegPostprocess> postprocessor;
        ...
        TRITONSERVER_Error *Init(ModelState *model_state)
        {
            ...
            postprocessor = std::make_unique<yolo11_seg_postprocess::Yolo11SegPostprocess>(
                model_state->config);

            const auto &cfg = model_state->config;
            size_t max_output0_elements = static_cast<size_t>(cfg.max_batch_size) *
                                          8400 * (cfg.num_classes + 4 + cfg.num_masks);
        ...
        bool anchors_first)
    {
        ...
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "output0 must be 3-D [N, C, A] or [N, A, C] tensor");
        }

        int n = static_cast<int>(output0_shape[0]);
        int dim1 = static_cast<int>(output0_shape[1]);
        int dim2 = static_cast<int>(output0_shape[2]);
        int expected_channels = num_classes + 4 + num_masks;

        if (anchors_first)
        {
            if (dim2 != expected_channels)
            {
                RETURN_TRITON_ERROR(INVALID_ARG, "output0 last dim must equal 4 + num_classes + num_masks");
            }
        ...
        else
        {
            ...
            {
                RETURN_TRITON_ERROR(INVALID_ARG, "output0 channel dim must equal 4 + num_classes + num_masks");
            }
        ...
        int total_elements = n * info.num_anchors * expected_channels;
    ...
    extern "C"
    {
            ...
            const uint32_t request_count)
        {
            ...
            yolo11_seg_postprocess::Yolo11SegPostprocess *postprocessor = instance_state->postprocessor.get();
            ...
            for (uint32_t r = 0; r < request_count; ++r)
            {
                ...
                TRITONSERVER_Error *err = ExtractModelOutputsFromRequest(
                    requests[r], info, config.num_classes, config.num_masks, config.anchors_first);
