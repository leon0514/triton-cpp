...
#define RETURN_TRITON_ERROR(CODE, MSG) \
        ...
        const yolo26_pose_postprocess::Yolo26PosePostprocessConfig &config)
    {
        ...
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "model_output must be 3-D [N, P, 6+K*D] tensor");
        }
        ...
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "model_output last dim must equal 6 + num_keypoints * keypoint_dim");
        }
            ...
            for (const auto &info : infos)
            {
                ...
                if (actual_num_dets > 0)
                {
                    ...
                    GUARDED_RETURN_IF_ERROR(CopyOutputStrided2D(
                        ...
                        actual_num_dets * kpt_stride * sizeof(float),
                        max_detections * kpt_stride * sizeof(float),
                        actual_num_dets * kpt_stride * sizeof(float),
                        info.batch_size,
