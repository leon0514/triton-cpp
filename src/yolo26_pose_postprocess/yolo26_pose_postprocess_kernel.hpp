...
#ifndef __YOLO26_POSE_POSTPROCESS_KERNEL_HPP__
#define __YOLO26_POSE_POSTPROCESS_KERNEL_HPP__
...
// YOLO26 one-to-one (NMS-free) pose 头输出的单个候选
// 输入格式: [batch, num_predictions, 6 + num_keypoints * keypoint_dim]
// 前 6 列: [x1, y1, x2, y2, score, class]
// 后 num_keypoints*keypoint_dim 列为关键点数据
struct Candidate
{
    float x1      = 0.0f;
    float y1      = 0.0f;
...
/**
 * @brief 对 YOLO26 one-to-one (NMS-free) pose 头输出做置信度过滤与 top-K 排序。
 *
 * 输入格式: [batch, num_predictions, 6 + num_keypoints * keypoint_dim]
 * 前 6 列含义: [x1, y1, x2, y2, score, class]
 *
 ...
 * @param keypoint_dim   每个关键点的维度
 * @param conf_thresh    置信度阈值
 * @param max_detections 每张图最多保留的检测框数
 * @param d_counts       每张图过滤后的候选数（device int[total_images]，中间缓冲区）
 * @param d_keypoints    候选框关键点缓冲区（device float[total_images * num_predictions * num_keypoints * keypoint_dim]）
 ...
 * @param d_sort_keys_in        CUB 排序输入 key 缓冲区
 * @param d_sort_keys_out       CUB 排序输出 key 缓冲区
 * @param d_sort_candidates_in  CUB 排序输入 value 缓冲区
 * @param d_sort_candidates_out CUB 排序输出 value 缓冲区
 * @param d_sort_offsets        分段排序偏移
...
#endif // __YOLO26_POSE_POSTPROCESS_KERNEL_HPP__
