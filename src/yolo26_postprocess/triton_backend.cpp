...
#include "yolo26_postprocess/triton_config.hpp"
...
#define BACKEND_NAME "yolo26_postprocess"
...
struct ModelState
{
    ...
    TRITONSERVER_Error *LoadConfig()
    {
        ...
        TRITONSERVER_Error *err = ParseYolo26PostprocessConfig(config_message, config);
...
struct ModelInstanceState
{
    ...
    TRITONSERVER_Error *Init(ModelState *model_state)
    {
        ...
        postprocessor = std::make_unique<yolo26_postprocess::Yolo26Postprocess>(
            model_state->config);

        // 预分配输入 workspace：max_batch * 300 * 6 * FP32
        const auto &cfg = model_state->config;
        size_t max_input_elements = static_cast<size_t>(cfg.max_batch_size) * 300 * 6;
    ...
    const uint32_t request_count)
{
    ...
    yolo26_postprocess::Yolo26Postprocess *postprocessor = instance_state->postprocessor.get();
