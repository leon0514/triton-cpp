...
// 后处理配置
struct Yolo11PostprocessConfig
{
    int num_classes          = 80;
    ...
    // 模型输出排布：
    // false -> [batch, 4+num_classes, num_anchors]
    // true  -> [batch, num_anchors, 4+num_classes]
...
{
  public:
    explicit Yolo11Postprocess(const Yolo11PostprocessConfig &config);
    ~Yolo11Postprocess() = default;

    Yolo11Postprocess(const Yolo11Postprocess &) = delete;
    Yolo11Postprocess &operator=(const Yolo11Postprocess &) = delete;

    /**
     * @brief 在指定 CUDA 流上执行 YOLO11 后处理。
     *
  ...
  private:
    Yolo11PostprocessConfig config_;

    tensor::Memory<int> counts_memory_;
    tensor::Memory<yolo11_postprocess::Candidate> candidates_memory_;
...
#endif // __YOLO11_POSTPROCESS_IMPL_HPP__
