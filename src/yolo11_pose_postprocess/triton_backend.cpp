...
#define RETURN_TRITON_ERROR(CODE, MSG) \
    ...
    struct ModelInstanceState
    {
        ...
        TRITONSERVER_Error *Init(ModelState *model_state)
        {
            ...
            size_t max_input_elements = static_cast<size_t>(cfg.max_batch_size) *
                                        8400 * (4 + cfg.num_classes + cfg.num_keypoints * cfg.keypoint_dim);
            input_workspace_.gpu(max_input_elements * sizeof(float));

            // 预分配 CUDA 锁页主机内存，用于快速拷贝每幅图像的实际检测数量。
            cuerr = cudaMallocHost(
                reinterpret_cast<void **>(&h_num_dets_pinned),
                cfg.max_batch_size * sizeof(int));
            if (cuerr != cudaSuccess)
            {
                RETURN_TRITON_ERROR(INTERNAL, cudaGetErrorString(cuerr));
            }
        ...
        const yolo11_pose_postprocess::Yolo11PosePostprocessConfig &config)
    {
        ...
        if (config.anchors_first)
        {
            ...
            {
                RETURN_TRITON_ERROR(INVALID_ARG, "model_output last dim must equal 4 + num_classes + num_keypoints * keypoint_dim");
            }
        ...
        else
        {
            ...
            {
                RETURN_TRITON_ERROR(INVALID_ARG, "model_output channel dim must equal 4 + num_classes + num_keypoints * keypoint_dim");
            }
    ...
    // pitch than the destination (e.g. workspace [N, max_dets, dim] with
    // stride max_dets*dim, but output only needs [N, actual_dets, dim]).
    static TRITONSERVER_Error *
    CopyOutputStrided2D(
        void *dst,
        const void *src,
    ...
    {                                    \
        ...
        {                                \
            ...
            if (static_cast<size_t>(total_images) > instance_state->pinned_capacity)
            {
                ...
                cudaError_t alloc_err = cudaMallocHost(
            ...
            for (int i = 0; i < request_num; ++i)
            {
                ...
                const int64_t boxes_shape[3] = {
                    infos[i].batch_size, actual_num_dets, 4};
