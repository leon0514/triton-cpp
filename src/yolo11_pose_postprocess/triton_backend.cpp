...
#define BACKEND_NAME "yolo11_pose_postprocess"
...
struct ModelState
{
    ...
    TRITONSERVER_Error *LoadConfig()
    {
        ...
        TRITONSERVER_Error *err = ParseYolo11PosePostprocessConfig(config_message, config);
...
struct ModelInstanceState
{
    ...
    int device_id = 0;
    std::unique_ptr<yolo11_pose_postprocess::Yolo11PosePostprocess> postprocessor;
    ...
    TRITONSERVER_Error *Init(ModelState *model_state)
    {
        ...
        postprocessor = std::make_unique<yolo11_pose_postprocess::Yolo11PosePostprocess>(
            model_state->config);

        // 预分配输入 workspace：max_batch * 8400 * (4+num_classes+num_keypoints*keypoint_dim) * FP32
        const auto &cfg = model_state->config;
        size_t max_input_elements = static_cast<size_t>(cfg.max_batch_size) *
                                    8400 * (4 + cfg.num_classes +
                                            cfg.num_keypoints * cfg.keypoint_dim);
...
ExtractModelOutputFromRequest(
    ...
    RequestInfo &info,
    const yolo11_pose_postprocess::Yolo11PosePostprocessConfig &config)
{
    ...
    {
        RETURN_TRITON_ERROR(INVALID_ARG, "Input must be 3-D [N, C, A] or [N, A, C] tensor");
    }

    int n = static_cast<int>(input_shape[0]);
    int dim1 = static_cast<int>(input_shape[1]);
    int dim2 = static_cast<int>(input_shape[2]);
    int expected_channels = 4 + config.num_classes +
                            config.num_keypoints * config.keypoint_dim;

    if (config.anchors_first)
    {
        if (dim2 != expected_channels)
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "Input last dim must equal 4 + num_classes + num_keypoints * keypoint_dim");
        }
    ...
    else
    {
        ...
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "Input channel dim must equal 4 + num_classes + num_keypoints * keypoint_dim");
        }
    ...
    const uint32_t request_count)
{
    ...
    yolo11_pose_postprocess::Yolo11PosePostprocess *postprocessor = instance_state->postprocessor.get();
    ...
    for (int i = 0; i < request_num; ++i)
    {
        ...
        const int64_t boxes_shape[3]    = {
            infos[i].batch_size, max_detections, 4};
        ...
        const int64_t keypoints_shape[4] = {
            infos[i].batch_size, max_detections, num_keypoints, keypoint_dim};
        ...
        const size_t keypoints_bytes = infos[i].batch_size * max_detections *
                                       num_keypoints * keypoint_dim * sizeof(float);
    ...
    // 7. 将结果从预分配 workspace 分发到各 response 的 output buffer
    for (const auto &info : infos)
    {
        ...
        GUARDED_RETURN_IF_ERROR(CopyOutputToResponse(
            ...
            info.batch_size * max_detections * num_keypoints * keypoint_dim * sizeof(float),
