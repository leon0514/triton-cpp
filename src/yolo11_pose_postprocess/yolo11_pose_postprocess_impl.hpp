...
// 后处理配置
struct Yolo11PosePostprocessConfig
{
    int num_classes          = 1;
    ...
    // 模型是否已经对关键点 visibility 做过 sigmoid
    bool apply_sigmoid_keypoints = false;

    // 模型输出排布：
    // false -> [batch, 4+num_classes+num_keypoints*keypoint_dim, num_anchors]
    // true  -> [batch, num_anchors, 4+num_classes+num_keypoints*keypoint_dim]
    bool anchors_first = false;
};

class Yolo11PosePostprocess
{
  public:
    explicit Yolo11PosePostprocess(const Yolo11PosePostprocessConfig &config);
    ~Yolo11PosePostprocess() = default;

    Yolo11PosePostprocess(const Yolo11PosePostprocess &) = delete;
    Yolo11PosePostprocess &operator=(const Yolo11PosePostprocess &) = delete;

    /**
     * @brief 在指定 CUDA 流上执行 YOLO11-pose 后处理。
     *
     * 输出缓冲区为实例初始化时预分配的 GPU workspace，调用者通过
     * num_detections_gpu() / boxes_gpu() / scores_gpu() / classes_gpu() / keypoints_gpu()
  ...
  private:
    Yolo11PosePostprocessConfig config_;

    tensor::Memory<int> counts_memory_;
    tensor::Memory<yolo11_pose_postprocess::Candidate> candidates_memory_;
...
#endif // __YOLO11_POSE_POSTPROCESS_IMPL_HPP__
