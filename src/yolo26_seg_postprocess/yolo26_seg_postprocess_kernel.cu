    ...
    cudaStream_t stream)
{
    ...
    if (input_is_half)
    {
        compute_and_crop_masks_kernel<<<grid_crop, 256, shared_mem_bytes, stream>>>(
            reinterpret_cast<const half *>(input),
            reinterpret_cast<const half *>(mask_protos),
            d_num_dets, d_candidates,
            total_images, num_predictions, num_masks,
            proto_h, proto_w, input_width, input_height,
            max_detections, num_predictions,
    ...
    else
    {
        compute_and_crop_masks_kernel<<<grid_crop, 256, shared_mem_bytes, stream>>>(
            reinterpret_cast<const float *>(input),
            reinterpret_cast<const float *>(mask_protos),
            d_num_dets, d_candidates,
            total_images, num_predictions, num_masks,
            proto_h, proto_w, input_width, input_height,
            max_detections, num_predictions,
