...
static bool GetBoolParameter(const nlohmann::json &parameters, const std::string &key, bool default_value)
{
    auto it = parameters.find(key);
    if (it == parameters.end())
    {
    ...
    if (param.contains("string_value"))
    {
        std::string v = param["string_value"].get<std::string>();
    ...
    rfdetr_seg_postprocess::RfDetrSegPostprocessConfig &config)
{
    ...
    try
    {
        ...
        config.return_masks = GetBoolParameter(parameters, "return_masks", config.return_masks);
