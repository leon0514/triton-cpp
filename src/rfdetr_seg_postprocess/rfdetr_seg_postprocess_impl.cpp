    ...
    const float *d2i)
{
    ...
    fprintf(stderr,
            "[rfdetr_seg_postprocess] conf_thresh=%f input_size=%.0fx%.0f mask_size=%dx%d "
            "max_detections=%d return_masks=%d\n",
            config_.confidence_threshold, config_.input_width, config_.input_height,
            mask_height, mask_width, config_.max_detections,
            static_cast<int>(config_.return_masks));
    fflush(stderr);
