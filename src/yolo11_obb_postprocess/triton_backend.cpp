    ...
    const uint32_t request_count)
{
    ...
    for (int i = 0; i < request_num; ++i)
    {
        ...
        const int64_t boxes_shape[3]    = {
            infos[i].batch_size, max_detections, 5}; // cx, cy, w, h, angle
        ...
        const size_t boxes_bytes    = infos[i].batch_size * max_detections * 5 * sizeof(float);
    ...
    for (const auto &info : infos)
    {
        ...
        GUARDED_RETURN_IF_ERROR(CopyOutputToResponse(
            ...
            info.batch_size * max_detections * 5 * sizeof(float),
            info.boxes_mem_type,
            stream));
