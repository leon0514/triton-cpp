...
#include "yolo11_obb_postprocess/triton_config.hpp"
...
#define BACKEND_NAME "yolo11_obb_postprocess"
...
#define RETURN_TRITON_ERROR(CODE, MSG) \
    ...
    struct ModelState
    {
        ...
        TRITONSERVER_Error *LoadConfig()
        {
            ...
            TRITONSERVER_Error *err = ParseYolo11ObbPostprocessConfig(config_message, config);
    ...
    struct ModelInstanceState
    {
        ...
        int device_id = 0;
        std::unique_ptr<yolo11_obb_postprocess::Yolo11ObbPostprocess> postprocessor;
        ...
        TRITONSERVER_Error *Init(ModelState *model_state)
        {
            ...
            postprocessor = std::make_unique<yolo11_obb_postprocess::Yolo11ObbPostprocess>(
                model_state->config);

            // 预分配输入 workspace：max_batch * 8400 * (5+num_classes) * FP32
            const auto &cfg = model_state->config;
            size_t max_input_elements = static_cast<size_t>(cfg.max_batch_size) *
                                        8400 * (cfg.num_classes + 5);
            input_workspace_.gpu(max_input_elements * sizeof(float));

            // 预分配页锁定 host buffer，用于拷贝实际检测数量
        ...
        bool anchors_first)
    {
        ...
        int expected_channels = num_classes + 5; // cx, cy, w, h, angle + classes

        if (anchors_first)
        {
            if (dim2 != expected_channels)
            {
                RETURN_TRITON_ERROR(INVALID_ARG, "model_output last dim must equal 5 + num_classes");
            }
        ...
        else
        {
            ...
            {
                RETURN_TRITON_ERROR(INVALID_ARG, "model_output channel dim must equal 5 + num_classes");
            }
    ...
    extern "C"
    {
            ...
            const uint32_t request_count)
        {
            ...
            yolo11_obb_postprocess::Yolo11ObbPostprocess *postprocessor = instance_state->postprocessor.get();
        ...
        {                                \
            ...
            for (int i = 0; i < request_num; ++i)
            {
                ...
                const int64_t boxes_shape[3] = {
                    infos[i].batch_size, actual_num_dets, 5}; // cx, cy, w, h, angle
                ...
                const size_t boxes_bytes = infos[i].batch_size * actual_num_dets * 5 * sizeof(float);
            ...
            for (const auto &info : infos)
            {
                ...
                if (actual_num_dets > 0)
                {
                    ...
                    GUARDED_RETURN_IF_ERROR(CopyOutputStrided2D(
                        info.boxes_buffer,
                        d_boxes + offset * max_detections * 5,
                        actual_num_dets * 5 * sizeof(float),
                        max_detections * 5 * sizeof(float),
                        actual_num_dets * 5 * sizeof(float),
                        info.batch_size,
                        info.boxes_mem_type,
                        stream));
