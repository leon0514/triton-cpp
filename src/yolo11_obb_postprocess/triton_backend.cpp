    ...
    bool anchors_first)
{
    ...
    if (anchors_first)
    {
        ...
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "Input last dim must equal 5 + num_classes");
        }
    ...
    else
    {
        ...
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "Input channel dim must equal 5 + num_classes");
        }
    ...
    for (const auto &info : infos)
    {
        ...
        if (actual_num_dets > 0)
        {
            GUARDED_RETURN_IF_ERROR(CopyOutputToResponse(
                ...
                info.batch_size * actual_num_dets * 5 * sizeof(float),
                info.boxes_mem_type,
                stream));
