...
struct ModelState
{
    ...
    TRITONSERVER_Error *LoadConfig()
    {
        ...
        TRITONSERVER_Error *err = ParseYolov5PostprocessConfig(config_message, config);
...
struct ModelInstanceState
{
    ...
    int device_id = 0;
    std::unique_ptr<yolov5_postprocess::Yolov5Postprocess> postprocessor;
    ...
    TRITONSERVER_Error *Init(ModelState *model_state)
    {
        ...
        postprocessor = std::make_unique<yolov5_postprocess::Yolov5Postprocess>(
            model_state->config);

        // 预分配输入 workspace：max_batch * 8400 * C * FP32
        // C = num_classes + 5（YOLOv5 经典格式含 objectness）
        const auto &cfg = model_state->config;
        int num_channels = cfg.num_classes + (cfg.has_objectness ? 5 : 4);
    ...
    bool has_objectness)
{
    ...
    int expected_channels = num_classes + (has_objectness ? 5 : 4);

    if (anchors_first)
    {
        if (dim2 != expected_channels)
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "Input last dim must equal C = num_classes + (has_objectness ? 5 : 4)");
        }
    ...
    else
    {
        ...
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "Input channel dim must equal C = num_classes + (has_objectness ? 5 : 4)");
        }
    ...
    const uint32_t request_count)
{
    ...
    yolov5_postprocess::Yolov5Postprocess *postprocessor = instance_state->postprocessor.get();
    ...
    for (uint32_t r = 0; r < request_count; ++r)
    {
        ...
        TRITONSERVER_Error *err = ExtractModelOutputFromRequest(
            requests[r], info, config.num_classes, config.anchors_first, config.has_objectness);
