...
// 移动另一个 Memory 对象的内存（转移所有权，源对象置空）
void BaseMemory::move_from(BaseMemory& other)
{
    // 1. 转移智能指针所有权
    this->cpu_ptr_ = std::move(other.cpu_ptr_);
    this->gpu_ptr_ = std::move(other.gpu_ptr_);
