...
#define BACKEND_NAME "yolo26_obb_postprocess"
...
#define RETURN_TRITON_ERROR(CODE, MSG) \
    ...
    struct ModelState
    {
        ...
        TRITONSERVER_Error *LoadConfig()
        {
            ...
            TRITONSERVER_Error *err = ParseYolo26ObbPostprocessConfig(config_message, config);
    ...
    struct ModelInstanceState
    {
        ...
        int device_id = 0;
        std::unique_ptr<yolo26_obb_postprocess::Yolo26ObbPostprocess> postprocessor;
        ...
        TRITONSERVER_Error *Init(ModelState *model_state)
        {
            ...
            postprocessor = std::make_unique<yolo26_obb_postprocess::Yolo26ObbPostprocess>(
                model_state->config);

            // 预分配输入 workspace：max_batch * 300 * 7 * FP32
            const auto &cfg = model_state->config;
            size_t max_input_elements = static_cast<size_t>(cfg.max_batch_size) * 300 * 7;
        ...
        RequestInfo &info)
    {
        ...
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "model_output must be 3-D [N, P, 7] tensor");
        }
        ...
        if (feat != 7)
        {
            RETURN_TRITON_ERROR(INVALID_ARG, "model_output last dim must equal 7");
        }
    ...
    extern "C"
    {
            ...
            const uint32_t request_count)
        {
            ...
            yolo26_obb_postprocess::Yolo26ObbPostprocess *postprocessor = instance_state->postprocessor.get();
...
#define GUARDED_RETURN_IF_ERROR(X)       \
    ...
    {                                    \
        ...
        {                                \
            ...
            if (request_num == 1 && infos[0].input_on_device && infos[0].input_mem_type_id == device_id)
            {
            ...
            for (int i = 0; i < request_num; ++i)
            {
                ...
                const int64_t boxes_shape[3] = {infos[i].batch_size, actual_num_dets, 5};
