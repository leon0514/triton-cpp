...
Yolo26ObbPostprocess::Yolo26ObbPostprocess(const Yolo26ObbPostprocessConfig &config)
    : config_(config)
{
...
void Yolo26ObbPostprocess::forward(
    const void *input,
    bool input_is_half,
    int total_images,
    int num_predictions,
    cudaStream_t stream)
{
    ...
    fprintf(stderr, "[yolo26_obb_postprocess] conf_thresh=%f max_detections=%d\n",
            config_.confidence_threshold, config_.max_detections);
    fflush(stderr);

    yolo26_obb_postprocess_gpu(
        input,
        input_is_half,
        total_images,
        num_predictions,
