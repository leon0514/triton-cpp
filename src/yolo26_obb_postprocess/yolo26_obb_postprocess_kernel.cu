    ...
    int *counts)
{
    ...
    int offset = (batch_idx * num_predictions + pred_idx) * 7;
    float cx    = static_cast<float>(input[offset + 0]);
    float cy    = static_cast<float>(input[offset + 1]);
    float w     = static_cast<float>(input[offset + 2]);
    float h     = static_cast<float>(input[offset + 3]);
    float score = static_cast<float>(input[offset + 4]);
    int class_id = __float2int_rn(static_cast<float>(input[offset + 5]));
    float angle = static_cast<float>(input[offset + 6]);
    ...
    int *classes)
{
    ...
    for (int i = 0; i < keep; ++i)
    {
        ...
        boxes[out_idx * 5 + 0] = c.cx;
        boxes[out_idx * 5 + 1] = c.cy;
        boxes[out_idx * 5 + 2] = c.w;
        boxes[out_idx * 5 + 3] = c.h;
        boxes[out_idx * 5 + 4] = c.angle;
