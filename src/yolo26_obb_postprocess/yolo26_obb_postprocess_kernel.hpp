...
#ifndef __YOLO26_OBB_POSTPROCESS_KERNEL_HPP__
#define __YOLO26_OBB_POSTPROCESS_KERNEL_HPP__
...
// YOLO26 one-to-one (NMS-free) OBB 头输出的单个候选框
// 输入格式: [batch, num_predictions, 7]
// 7 列含义: [cx, cy, w, h, score, class, angle]
struct Candidate
{
    float cx      = 0.0f;
    float cy      = 0.0f;
...
/**
 * @brief 对 YOLO26 one-to-one (NMS-free) OBB 头输出做置信度过滤与 top-K 排序。
 *
 * 输入格式: [batch, num_predictions, 7]
 * 7 列含义: [cx, cy, w, h, angle, score, class]
 *
...
void yolo26_obb_postprocess_gpu(
    const void *input,
    bool input_is_half,
    int total_images,
    int num_predictions,
...
#endif // __YOLO26_OBB_POSTPROCESS_KERNEL_HPP__
