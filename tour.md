...
3. [项目结构一览](#3-项目结构一览)
4. [Backend 生命周期与 C API](#4-backend-生命周期与-c-api)
5. [最小可运行 Backend 示例](#5-最小可运行-backend-示例)
6. [核心机制详解](#6-核心机制详解)
   - 6.1 [状态管理：ModelState 与 ModelInstanceState](#61-状态管理-modelstate-与-modelinstancestate)
   - 6.2 [执行入口：ModelInstanceExecute](#62-执行入口-modelinstanceexecute)
   - 6.3 [Request/Response 与 Tensor](#63-requestresponse-与-tensor)
   - 6.4 [内存管理与异步响应](#64-内存管理与异步响应)
7. [CUDA 集成最佳实践](#7-cuda-集成最佳实践)
8. [config.pbtxt 与参数解析](#8-configpbtxt-与参数解析)
9. [实战：preprocess backend](#9-实战-preprocess-backend)
10. [实战：yolo11_postprocess backend](#10-实战-yolo11_postprocess-backend)
11. [实战：yolov5_postprocess backend](#11-实战-yolov5_postprocess-backend)
12. [构建与部署](#12-构建与部署)
...
│   ├── common/                 # 公共代码
│   │   ├── check.hpp           # CUDA 检查宏
│   │   ├── device.hpp/.cpp     # AutoDevice
│   │   ├── memory.hpp/.cu      # GPU/CPU 内存池
│   │   ├── affine.hpp          # 仿射变换
│   │   └── json.hpp            # nlohmann/json
│   ├── preprocess/             # 预处理后端
│   │   ├── preprocess_kernel.cu
│   │   ├── preprocess_impl.cpp/.hpp
│   │   ├── triton_config.cpp/.hpp
│   │   └── triton_backend.cpp
│   ├── yolo11_postprocess/     # YOLO11 后处理后端
│   │   ├── yolo11_postprocess_kernel.cu
│   │   ├── yolo11_postprocess_impl.cpp/.hpp
│   │   ├── triton_config.cpp/.hpp
│   │   └── triton_backend.cpp
│   └── yolov5_postprocess/     # YOLOv5 后处理后端
│       ├── yolov5_postprocess_kernel.cu
│       ├── yolov5_postprocess_impl.cpp/.hpp
│       ├── triton_config.cpp/.hpp
│       └── triton_backend.cpp
├── tests/                      # 客户端测试脚本
└── workspace/
...
每个 backend 的代码组织遵循相同模式：

| 层级 | 文件 | 职责 |
|------|------|------|
| Triton 接口层 | `triton_backend.cpp` | 实现 C API，处理 request/response |
| 配置解析层 | `triton_config.cpp/.hpp` | 解析 `config.pbtxt` |
| C++ 封装层 | `*_impl.cpp/.hpp` | 管理 workspace，提供 `forward()` |
| CUDA 算法层 | `*_kernel.cu/.hpp` | GPU kernel |

---

## 4. Backend 生命周期与 C API
...
### 6.1 状态管理：ModelState 与 ModelInstanceState
...
## 10. 实战：yolo11_postprocess backend
...
## 11. 实战：yolov5_postprocess backend
